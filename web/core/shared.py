...
LLM_WORKSPACE_CHAT_AGENT: LLMWorkspaceAgent | None = None
LLM_WORKSPACE_CHAT_SIGNATURE: tuple[str, ...] | None = None
...
VIDEO_ANALYZE_SYSTEM_PROMPT = (
    "你是 B 站视频分析专家，对单个视频做闭环分析。\n"
    "规则：\n"
    "1. 只用本次请求数据+工具检索结果，禁止读长期记忆。禁用 video_briefing 和 hot_board_snapshot。\n"
    "2. 先调 retrieval（query 用 benchmark_queries 短查询），样本不足时再调 web_search。\n"
    "3. peer_samples 是预抓爆款对标，优先引用，禁止编造 BV 号。\n"
    "4. 输出：performance.score + is_hot 判定；爆款拆优点+延伸方向，低表现给全维度改进+新文案。\n"
    "5. 只返回 JSON，不输出 markdown。"
)
